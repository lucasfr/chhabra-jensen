% Authors: Lucas Franca(1), Yujiang Wang(1,2,3), Jose Garcia Vivas Miranda(4)
...
function [alpha,falpha,Dq,Rsqr_alpha,Rsqr_falpha,Rsqr_Dq,muScale,Md,Ma,Mf]=...
ChhabraJensen_2d(Matrix,qValues,scales)
%INPUT:
%Matrix is a square matrix with a dimensional in dyadic scale 
%qValues, and scales are all row vectors
...
%muScale, Md, Ma, and Mf are the basic matricies from which alpha, falpha, and Dq can be constructed by linear regression (see code below line 81 - 92 for details). They are included in the output mainly for completeness.
    ...
    for i = 1:nq
        ...
        for j = 1:ns
            ...
            %break the matrix into smaller ones & sum
            MatrixReshaped = mat2cell(Matrix, repmat(window, [1,...
                length(Matrix)/window]), repmat(window, [1, ...
                length(Matrix)/window]));
            
            MatrixSummed = sum(sum(Matrix));
            
            %calculate p
            ps = cellfun(@(x)sum(x(:)),MatrixReshaped,'un',0);
            ps = cell2mat(ps);
            ps = reshape(ps,1,[]);
            p=ps./MatrixSummed;

...
%%% FROM TIME SERIES BASED ON A CODE ORIGINALLY WRITTEN BY J. G. V. MIRANDA.
...
for q=qInitial:qVar:qFinal
 ...
 %% II - CALCULATING THE MEASURES
        ...
        % FOR q < 0 && q > 1 | LOG OF THE SUM (CHECK THIS)
        if(~(q > (1-qVar/2) && q < (1+qVar/2)))

            Md(int64((q-qInitial)/qVar)+1,k-initialScale+1)= ...
                log10(Md(int64((q-qInitial)/qVar)+1,k-initialScale+1));
